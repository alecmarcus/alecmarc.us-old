<template>
  <nav>
    <span
      >{{ projectArray.indexOf(activeProjectId) + 1 }} /
      {{ projectArray.length }}</span
    >
    <span>
      <nuxt-link :to="'/projects/' + prevProjectId" class="previous"
        >Previous</nuxt-link
      >
      •
      <nuxt-link :to="'/projects/' + nextProjectId" class="next"
        >Next</nuxt-link
      >
    </span>
  </nav>
